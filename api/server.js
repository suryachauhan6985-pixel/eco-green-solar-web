...
// free tier).
//
// --- Option A (in use / recommended — domain authenticated with Brevo): Brevo
//   BREVO_API_KEY, BREVO_FROM_EMAIL, BREVO_FROM_NAME — sends via Brevo's
//   HTTPS API (no SMTP port involved, so it works fine on Render's free
//   tier). Requires the sender/domain to be verified in Brevo and the
//   account's transactional-email capability to be activated by Brevo.
//
...
//   Used as the fallback if Brevo fails or is not configured.
//
